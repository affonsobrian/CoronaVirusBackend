...
from api.models import Profile, Notification, NotificationType, Answer, AnsweredQuestions, Question
...
class AnswerQuestionSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def validate(self, attrs):
        ...
        is_answered = AnsweredQuestions.objects.filter(user=user, question=question).exists()
        if is_answered:
            raise serializers.ValidationError({"user": "You already answered this questions!"})
        return attrs
